...
void send(uint elo, char h_rand, Bounds b = bounds ){ /* not yet implimented*/ }
void send(int size, char c){ /* not yet implimented*/ }
void send(map<int,IP> data, char c){ /* not yet implimented*/ }
void send(char c){/* end */}

char handshake_rand = 26;
void SYN()
{
  //send UDP packet 
  send(MY_ELO, handshake_rand, bounds);
}
...
/* map<int,IP> */void SYN_ACK(Packet q, uint lower_bound, uint upper_bound, char h_rand)
{
  map<int,IP> result = {};
  ...
  check_elo(q);
  // (int) result.size() * (IP.size() + Int.size())
  send((int) result.size() , h_rand+1);
  send(result, h_rand +2);
  // return result;
}
